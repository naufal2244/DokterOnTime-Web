...
<body>
                    ...
                    <div class="card-body">
                        <form name="add_form" class="inline-form" method="POST" action="<?= htmlspecialchars($_SERVER['PHP_SELF']); ?>" enctype="multipart/form-data">
                            ...
                            <div class="form-group row">
								<label for="inputSpeciality" class="col-sm-3 col-form-label text-right">Poli</label>
								<div class="col-sm-6">
									<input type="text" name="inputSpeciality" id="inputSpeciality" class="form-control form-control-sm">
								</div>
							</div>
                            <div class="form-group row">
								<label for="inputIcon" class="col-sm-3 col-form-label text-right">Ikon</label>
								<div class="col-sm-6">
							...
							<div class="d-flex justify-content-md-center pt-2">
								<button type="reset" class="btn btn-light btn-sm px-5 mr-2" name="clearbtn">Bersihkan Input</button>
								<button type="submit" class="btn btn-primary btn-sm px-5" name="savebtn">Tambah</button>
							</div>
						</form>
                    </div>
                </div>
                                    ...
                                    while ($table_row = mysqli_fetch_assoc($table_result)) {
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <button type="button" class="btn btn-sm btn-danger" data-toggle="modal" data-target="#deletemodal<?php echo $table_row["speciality_id"]; ?>"><i class="fa fa-trash"></i> Hapus</button>
                                            </td>
                                        </tr>
                                            ...
                                            <div class="modal-dialog modal-dialog-centered" role="document">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="ModalCenterTitle">Edit Poli</h5>
    ...
    <div class="modal-body">
        ...
        <small class="form-text text-muted">Nama Poli Sebelumnya : <?php echo $table_row["speciality_name"]; ?></small>
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <form action="<?= htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
                                                        <div class="modal-body">
                                                            ...
                                                            Apakah Anda yakin ingin menghapus <strong><?= $table_row['speciality_name'] ?></strong> ?
                                                        </div>
...
if (isset($_POST["updatebtn"])) {
    ...
    if (isset($_FILES["inputUpdateIcon"]["name"]) && !empty($_FILES["inputUpdateIcon"]["name"])) {
        $allowed = array('png', 'jpg', 'jpeg');
    ...
    if (!$result = $conn->query($sql)) {
        ...
        echo "<script>Swal.fire({ title: 'Berhasil!', text: '$update Telah Diperbarui', type: 'success' }).then(function() { location.href = '$current_link'; });</script>";
    }
}
...
if (isset($_POST["deletebtn"])) {
    ...
    if (!$result = $conn->query($sql)) {
        ...
        echo "<script>Swal.fire({ title: 'Berhasil!', text: 'Berhasil Hapus Data', type: 'success' }).then(function() { location.href = '$current_link'; });</script>";
    }
}
?>