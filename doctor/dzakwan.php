...
if (isset($_POST['appointmentbtn'])) {
    ...
    if (count($apperrors) == 0) {
        $appstmt = $conn->prepare("INSERT INTO appointment (app_date, app_time, treatment_type, patient_id, clinic_id, doctor_id) VALUES (?,?,?,?,?,?) ");
...
if (isset($_POST["completebtn"])) {
    ...
    if ($comstmt->execute()) {
        echo '<script>
            Swal.fire({ title: "Great!", text: "Successfully!", icon: "success" }).then((result) => {
    ...
    <?php if ($success): ?>
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = "appointment-view.php?id=" + encryptedId;
            }
        });
    </script>
    <?php endif; ?>
